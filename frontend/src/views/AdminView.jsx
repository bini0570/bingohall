...
// ─────────────────────────────────────────────────────────────
export default function AdminView({ lang, token, socket, onLogout, onToggleLang, currentLang }) {
  ...
  useEffect(() => {
    fetchAdminData();
    if (socket) {
      socket.on('admin_data_changed', fetchAdminData);
      return () => socket.off('admin_data_changed', fetchAdminData);
    }
  }, [token, socket]);
  ...
  const approveDeposit = async id => {
    ...
    const r = await fetch(`/api/admin/deposits/${id}/approve`, { method:'POST', headers:{Authorization:`Bearer ${token}`} });
  ...
  const approveWithdrawal = async id => {
    ...
    const r = await fetch(`/api/admin/withdrawals/${id}/approve`, { method:'POST', headers:{Authorization:`Bearer ${token}`} });
    const data = await r.json();
    if (!r.ok) { flash('error', data.error); fetchAdminData(); }
  ...
  const submitReject = async () => {
    ...
    await fetch(`/api/admin/${type==='deposit'?'deposits':'withdrawals'}/${id}/reject`,{
  ...
  const toggleBan = async userId => {
    await fetch(`/api/admin/users/${userId}/ban`,{ method:'POST', headers:{Authorization:`Bearer ${token}`} });
    fetchAdminData();
  };
    ...
    <div style={S.shell}>
      ...
      <div style={S.topbar}>
        ...
        <span style={S.topbarTitle}>
          {NAV.find(n=>n.key===activeTab)?.label || 'Admin Panel'}
        </span>
        ...
        <div style={{display:'flex',alignItems:'center',gap:'8px',flexShrink:0}}>
          {onToggleLang && (
            <button
              style={{...S.btnGhost,padding:'6px 10px',fontSize:'12px',whiteSpace:'nowrap'}}
            ...
            return (
              <>
                ...
                <div style={metricsGrid}>
                  ...
                  {/* Payouts = total approved withdrawals */}
                ...
                <div style={{...S.row('space-between','10px',true),marginBottom:'12px'}}>
                  <div>
                    ...
                    <div style={{fontSize:'22px',fontWeight:'900',color:'#fff',display:'flex',alignItems:'center',gap:'10px'}}>
                      ...
                      Round #{metrics?.currentRoundId || 1}
                    </div>
                  </div>
                ...
                <div key={d.id} style={{
                  ...
                  {d.status==='pending' && (
                    <div style={{...S.row('flex-start','8px'),flexWrap:'wrap'}}>
                      {d.proof_image && (
                        <a href={d.proof_image} target="_blank" rel="noreferrer"
