...
function SettingsTab({ settings, setSettings, token, flash }) {
  ...
  return (
    <div className="admin-card" style={{ maxWidth: '500px' }}>
      <div className="admin-card-header">Game Settings</div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
        <div>
          <label style={{ display: 'block', fontSize: '12px', color: '#64748b', marginBottom: '6px' }}>Ticket Price (ETB)</label>
        ...
        <div>
          <label style={{ display: 'block', fontSize: '12px', color: '#64748b', marginBottom: '6px' }}>Countdown Duration (sec)</label>
          <input className="admin-input" type="number" value={settings.countdown_sec||''} onChange={e=>handleChange('countdown_sec', e.target.value)} />
        </div>
        <div>
          <label style={{ display: 'block', fontSize: '12px', color: '#64748b', marginBottom: '6px' }}>Draw Speed (sec per ball)</label>
        ...
        <button className="admin-btn admin-btn-primary" style={{ marginTop: '8px' }} onClick={save}>Save Settings</button>
      </div>
    </div>
  );
}
