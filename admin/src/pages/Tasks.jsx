...
export default function Tasks() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setCreating(false);
      fetchTasks();
      setFormData({ type: 'telegram', title: '', telegram_link: '', button_name: 'Join Channel', reward: 10, target: 'All Players' });
    } catch (err) {
        ...
        {tasks.map(task => (
          <Card key={task.id} className={cn("p-5 flex flex-col justify-between transition-opacity", task.status !== 'active' && 'opacity-60')}>
            <div>
              ...
              <p className="text-sm text-gray-500 line-clamp-1">{task.telegram_link}</p>
